...
public class CrackerHandler extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		//setting RmiRequestHandler to use the Queue and Map from CrackerWorker
		rrh = new RmiRequestHandler(worker.getInQueue(), worker.getOutQueue());
		Thread t = new Thread(rrh);//starting a new thread
		t.start();
	}
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (taskNumber == null){
			...
			jobNumber++;
			//Add job to in-queue
			//creating a message object from input
			msObject = new MessageObject(jobNumber, cypherText, maxKeyLength);
			worker.add(msObject);//adding object to in-queue

			/*
			 * Cypher text tested, Key used MOON
			 * 
			 * 	DOGZURRIOHYSOGYSPWJGKFBSJU
				EODPNSEMDWJKUBF
				EHNYNYJ
				When the cypertext is equal to the key used it does not work
				ICFX
			 * 			 
			 * */
			...
			try {
				returnText = worker.get(jobNumber);//taking the jobNumber from the map
			} catch (Exception e) {
		...
		out.println("<P>New Object created from input: ");
		out.print("<P>Jobnumber: "+msObject.getJobNumber()+" Cypertext: "+msObject.getCyperText()+" Max Key Length: "+msObject.getMaxKeyLength());
